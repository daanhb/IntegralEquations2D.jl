...
function DomainIntegrals.integrate_domain(qs::QuadQBF, integrand, domain::PeriodicInterval, measure, properties...)
    if ncomponents(domain) == 1
        DomainIntegrals.integrate_domain(qs, integrand, component(domain, 1), measure, properties...)
    else
        # only periodize when crossing the boundary
        A, B = extrema(domain.periodicdomain)
        DomainIntegrals.integrate_domain(qs, integrand ∘ periodizer(A,B), domain.subdomain, measure, properties...)
    end
end
...
function DomainIntegrals.splitdomain_point(x, domain::PeriodicInterval)
    if ncomponents(domain) == 1
        DomainIntegrals.splitdomain_point(x, component(domain, 1))
    else
        vcat(DomainIntegrals.splitdomain_point(x, component(domain, 1))..., DomainIntegrals.splitdomain_point(x, component(domain, 2))...)
    end
end
...
function piecewisedomains(domain::PeriodicInterval, order)
    ...
    vcat([collect(components(PeriodicInterval(s, periodicdomain))) for s in segments]...)
end
...
function doubleprojection(qs, f, dict1, idx1, measure1, dict2, idx2, measure2, sing,
    ...
    sum(doubleprojection(qs, f, dict1, idx1, measure1, dict2, idx2, measure2, sing, d1, d2)
            for d1 in components(domain1), d2 in components(domain2))
end
