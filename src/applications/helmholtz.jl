...
function hh2_slp(x, y, wavenumber)
    T = eltype(x)
    if norm(x-y) < eps(T)
...
"Evaluate the Helmholtz 3D single layer potential kernel."
function hh3_slp(x, y, wavenumber)
    T = promote_type(eltype(x), eltype(y), typeof(wavenumber))
    R = norm(x-y)
    if R < eps(T)
        zero(complex(T))
    else
...
hh2_dlp(x, y, wavenumber, normal_y, z = norm(x-y)) =
...
function hh2_dlp_limit(x, y, wavenumber, normal_y, grad_y, grad_d_y)
    -(grad_d_y[1] * grad_y[2] - grad_y[1] * grad_d_y[2]) / (4*norm(grad_y)^3*pi) +0im
end

"Evaluate the Helmholtz 2D double layer potential kernel"
function hh2_dlp_kernel(x, y, tau, wavenumber, param)
    z = norm(x-y)
    if abs(z) > 100*eps(eltype(x))
        hh2_dlp(x, y, wavenumber, normal(param, tau), z)
    else
        # Evaluate the limiting value
        hh2_dlp_limit(x, y, wavenumber, normal(param, tau),
            gradient(param, tau), gradient_derivative(param, tau))
    end
end

hh2_slpt(x, y, wavenumber, normal_x) = error("to do")
hh2_hypersingular(x, y, wavenumber, normal_x, normal_y) = error("to do")
...
(kernel::Helmholtz_SLP_2D)(t, tau, param, x, y) = hh2_slp(x, y, wavenumber(kernel))
(kernel::Helmholtz_SLP_2D)(x, tau, param, y) = hh2_slp(x, y, wavenumber(kernel))
